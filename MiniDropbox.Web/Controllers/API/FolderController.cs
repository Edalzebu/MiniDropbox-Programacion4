...
namespace MiniDropbox.Web.Controllers.API
{
    public class FolderController : ApiController
    {
        ...
        // POST api/folder
        public FolderModel Get([FromUri]string currentPath, [FromUri] string token) // Funcion para devolver cierto folder
        {
            var account = CheckPermissions(token);
            var model = new FolderModel();
        ...
        // PUT api/folder/5
        public FolderModel Put([FromUri] string currentPath, [FromUri] string folderName, [FromUri] string token) // funcion para crear un folder, devuelve la listqa del directorio
        {
            var account = CheckPermissions(token);
            if (CheckCuenta(account))
            {
                if (CreateFolder(currentPath, folderName, token))
                {
        ...
        private bool CreateFolder(string path,string folderName, string token)
        {
