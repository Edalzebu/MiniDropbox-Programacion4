$(function(){   
	...
	$('#btnAgregar').on('click',function(){
		...
		//con la funcion parseInt() convertimos de t|o a numero
		var $txtNombre=$('#txtNombre'), iLimite=0;
	...
	$('#txtNombre').on('keypress', function(eEvento) {
	    if (eEvento.which == 13) {
	        if ($('#txtNombre').val == "" || !emailreg.test($('#txtNombre').val())) {
	            $('#txtNombre').focus().after("<span class='error'>Ingrese un email correcto</span>");
	        } else {
	            //obtenemos el nombre digitado por el usuario, y el limite establecido
	            //con la funcion parseInt() convertimos de t|o a numero
	            var $txtNombre = $('#txtNombre'), iLimite = 0;
	            ...
	            if ($.trim($txtNombre.val()) != '') {
	                ...
	                alert('Por favor, digite el email que desea agregar a la lista.')
	            }
	            //limpiamos el campo nombre y lo enfocamos
	            $txtNombre.val('').focus();
	        }
